...
| `unvoiced-article` | The article names no `persona:` traits, in an archive that has some. Everything under `wiki/` is the clone's writing, so it should record which voice it was written through. **Read it and cite only traits it genuinely exhibits.** If it exhibits none — a flat summary of somebody else's paper often will not — say so in the report and leave it. Attaching plausible traits to make the warning go away is inventing the authority the article was written on, which is the same failure as `unattributed-extrapolation`. |
